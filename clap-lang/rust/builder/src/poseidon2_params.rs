...
pub const ALL_ROUND_CONSTANTS_AS_FIELD_ELEMENTS: [F; STATE_WIDTH * TOTAL_NUM_ROUNDS] = const {
...
pub const ROUND_CONSTANTS_ALIGNED_PER_ROUND_AS_FIELD_ELEMENTS: [[F; STATE_WIDTH];
    TOTAL_NUM_ROUNDS] = const { unsafe { std::mem::transmute(ROUND_CONSTANTS_ALIGNED_PER_ROUND) } };

pub const PARTIAL_ROUND_CONSTANTS: [F; NUM_PARTIAL_ROUNDS] = const {
    let mut constants_array = [F::ZERO; NUM_PARTIAL_ROUNDS];
    let mut i = 0;
    while i < NUM_PARTIAL_ROUNDS {
        constants_array[i] =
            ROUND_CONSTANTS_ALIGNED_PER_ROUND_AS_FIELD_ELEMENTS[HALF_NUM_FULL_ROUNDS + i][0];
        i += 1;
    }
    constants_array
};

pub const FULL_ROUND_CONSTANTS: [[F; STATE_WIDTH]; NUM_FULL_ROUNDS_TOTAL] = const {
    let mut constants_array = [[F::ZERO; STATE_WIDTH]; NUM_FULL_ROUNDS_TOTAL];
    let mut i = 0;
    while i < HALF_NUM_FULL_ROUNDS {
        constants_array[i] = ROUND_CONSTANTS_ALIGNED_PER_ROUND_AS_FIELD_ELEMENTS[i];
        i += 1;
    }
    let mut i = 0;
    while i < HALF_NUM_FULL_ROUNDS {
        constants_array[i + HALF_NUM_FULL_ROUNDS] =
            ROUND_CONSTANTS_ALIGNED_PER_ROUND_AS_FIELD_ELEMENTS
...
pub const INNER_ROUNDS_MATRIX_DIAGONAL_ELEMENTS_MINUS_ONE: [F; 12] = const {
