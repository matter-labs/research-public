...
open Eq using (_≡_; refl ; decSetoid ; subst)
...
-- 0 days since last proof of ⊥
_ : inverse (ᵢ+ 0) (ᵢ+ 2) ≡ ᵢ+ 0
_ = refl
...
-- Takes a circuit, an offset and a list of allowed input wires
...
WFseqconstconst : ∀ x → WellFormedCircuit
  (seq (const x) (const x)) (2 ,, ([]))
proj₁ (WFseqconstconst x) = tt
proj₂ (WFseqconstconst x) = tt
...
-- 
-- 
-- -- All used vars are within bound
-- WFCS : List Expr → (bound : ℕ ) → Set
-- WFCS [] n = ⊤
-- WFCS ((x + x₁ ＝ x₂) ∷ snd) n = (suc x ≤ n) × (suc x₁ ≤ n) × (suc x₂ ≤ n)  × WFCS (snd) n
-- WFCS ((x * x₁ ＝ x₂) ∷ snd) n = (suc x ≤ n) × (suc x₁ ≤ n) × (suc x₂ ≤ n)  × WFCS (snd) n
-- WFCS ((x ＝c x₁) ∷ snd) n = (suc x ≤ n) ×  WFCS (snd) n
-- 
-- -- Number of vars and constraints on them 
-- CS = ℕ × List Expr
-- 
-- lkp : ℕ → List ℤ → ℤ
-- lkp zero [] = ᵢ+ 0 -- What is this???
-- lkp zero (x ∷ l) = x
-- lkp (suc x) [] = ᵢ+ 0
-- lkp (suc x) (x₁ ∷ l) = lkp x l
-- 
-- {-# TERMINATING #-}
-- genTrace : Circuit → List ℤ → List ℤ -- Shouldn′t this be partial?
...
-- 
-- {-# TERMINATING #-}
-- -- What does genCS do? Well it does not generate constraint systems duh!
-- genCS : Circuit → CS → CS
